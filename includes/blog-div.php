<div class="col-xs-12 col-md-4 py-xs-5">
    <span>24/05/2019</span>
    <a href="blog/<?= cleanString($rowResult['titulo_post']); ?>-<?= $rowResult['id_post'] ?>"><h3>Tittle lorem ipsum sit depscur es</h3></a>
    <p>Lorem ipsum dolor sit, amet consectetur adipisicing elit. Corporis praesentium ut tenetur modi unde similique laudantium debitis sed, ea quo magnam ullam.</p>
    <div class="blog-saibaMais">
        <a href="blog-detalhe.php">Saiba mais</a>
    </div>
</div>